  ...
  try {
    ...
    await s3.send(
      new PutObjectCommand({
        Bucket: process.env.NEXT_PUBLIC_KIRA_AWS_S3_BUCKET_NAME,
        Key: key,
        Body: buffer,
      })
    );
    ...
    const command = new StartTranscriptionJobCommand({
      ...
      LanguageCode: "en-US",
      Media: {
        MediaFileUri: `s3://${process.env.NEXT_PUBLIC_KIRA_AWS_S3_BUCKET_NAME}/${key}`,
      },
      OutputBucketName: process.env.NEXT_PUBLIC_KIRA_AWS_S3_BUCKET_NAME,
